...
int color_off = 0xFFFFFFFF;
int enemy_color = 0x000000FF;
int enemy_color_off = 0xFFFFFFFF;
int score = 0;
int random = 0;
    ...
    switch(uMsg) {
        ...
        case WM_CREATE:
            ...
            case WM_PAINT:
                ...
                SetPixel(hdc, x_point, y_point, color_off); // params: SetPixel(hdc), x_position, y_position, color))
                SetPixel(hdc, x_point + 1, y_point + 1, color_off); // add diagonally from center point.
                SetPixel(hdc, x_point - 1, y_point - 1, color_off); // add inverted digonal from center point.
                SetPixel(hdc, x_point, y_point - 1, color_off); // direct above center point.
                SetPixel(hdc, x_point - 1, y_point, color_off); // left side of center.
                SetPixel(hdc, x_point + 1, y_point, color_off); // right side of center.
                SetPixel(hdc, x_point - 1, y_point + 1, color_off); // left top from center
                SetPixel(hdc, x_point + 1, y_point - 1, color_off); // right top
                SetPixel(hdc, x_point, y_point - 2, color_off); // body low
                SetPixel(hdc, x_point, y_point - 3,  color_off); // body high
                SetPixel(hdc, x_point, y_point - 4, color_off); // head
                SetPixel(hdc, x_point + 2, y_point - 2, color_off); // right arm
                SetPixel(hdc, x_point - 2, y_point - 2, color_off); // left arm
                SetPixel(hdc, enemy_x, enemy_y, enemy_color_off);
                SetPixel(hdc, enemy_x + 1, enemy_y, enemy_color_off);
                SetPixel(hdc, enemy_x - 1, enemy_y - 2, enemy_color_off);
                SetPixel(hdc, enemy_x + 1, enemy_y - 1, enemy_color_off);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    free(&color_off);free(&enemy_color_off);
	return 0;
